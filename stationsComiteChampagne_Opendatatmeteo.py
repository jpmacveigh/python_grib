#coding: utf8
''' Renvoi la température à 2m prévue demain matin à 05UTC par Arome 025 pour
    les stations du Comité Champagne (point de la grille 025 degrés le plus proche) '''
...
heures_UTC_prévisions=[5]  # liste des prochaines heures UTC recherchées 
code_model="arome-france-hd"  # modele Arome extrait sur la grille 025
code_param="TMP"           # température
code_type_niveau="2m"      # le niveau 2m sera extrait
for heure in heures_UTC_prévisions:   #  boucle sur les heures
    date_previ=next_hh_UTC(heure)[1]
    les_dates=les_dates_de_sortie_entourant_date_dans_le_temps (code_model,date_previ)
    if les_dates[0] :
        ...
        f=open(file_name,'rb')  # ouverture du fichier Grib en lecture seule
        nbGrib=codes_count_in_file(f)  # nombre de Grib dans le fichier
        assert (nbGrib==1)   # il doit être égal à 1 pour le seul niveau 2m demandé
        for i in range (nbGrib):  # boucle sur les Grib contenus dans le fichier
            ...
            for station in stationsComiteChampagne :   # boucle sur les stations
                val=codes_grib_find_nearest	(msg,station["lat"],station["lng"],is_lsm=False,npoints=1)  # extrcation de la valeur au ploint le plus proche de la station
                print (datepreviUTC,station["commune"],station["lieu-dit"],station["alt"],val[0]["distance"],val[0]["value"]-273.15)
            codes_release(msg)